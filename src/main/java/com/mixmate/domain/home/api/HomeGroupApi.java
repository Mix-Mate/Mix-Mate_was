...
public interface HomeGroupApi {
    ...
    @Operation(summary = "참여코드 검증",
            description = "입력한 참여코드가 실제로 존재하고 아직 참가자를 모집중인 그룹인지 확인합니다. "
                    + "프로필 입력 화면으로 넘어가기 전에 먼저 코드만 빠르게 검증할 때 사용합니다. "
                    + "모집이 마감된 그룹은 입장도 불가능하므로 이 단계에서 함께 거부합니다.")
    @ApiResponses({
            ...
            @ApiResponse(responseCode = "401", description = "인증 없음",
                    content = @Content(examples = @ExampleObject(value = """
                            ...
                            """))),
            @ApiResponse(responseCode = "404", description = "존재하지 않거나 만료된 참여코드",
                    content = @Content(examples = @ExampleObject(value = """
